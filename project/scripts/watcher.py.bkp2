...
DEBOUNCE_DELAY = 1  # Minimum delay (in seconds) between consecutive runs for the same file

class ChangeHandler(FileSystemEventHandler):
    """
    Handles file system events in the src directory.
    Triggers the merge process only for modified or created files, with debounce logic.
    """
    def on_any_event(self, event):
        ...
        if event.event_type in ["modified", "created"]:
            ...
            if file_path not in last_run_time or current_time - last_run_time[file_path] > DEBOUNCE_DELAY:
                ...
                run_merge(file_path)
                last_run_time[file_path] = current_time  # Update the last run time for this file
            # else:
...
def run_merge(changed_file):
    ...
    try:
        ...
        # Log that the watcher is waiting for the next change
...
if __name__ == "__main__":
    ...
    # Set up the observer to monitor only the src directory
    event_handler = ChangeHandler()
    observer = Observer()

    # Schedule the observer to watch the src directory
